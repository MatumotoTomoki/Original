...
bool GameCamera::Start() {
	...
	m_toCameraPos.Set(0.0f, 0.0f, -300.0f);
	...
	if (m_player) { // プレイヤーが見つかったことを確認
		m_currentCameraTarget = m_player->m_position;
		m_currentCameraTarget.y += 80.0f; // Y軸のオフセットも考慮
	}
	else {
		// プレイヤーが見つからなかった場合の初期値も設定しておくと安全だよ
		m_currentCameraTarget.Set(0.0f, 80.0f, 0.0f);
	}
...
void GameCamera::Update() {
	if (!m_player) { // プレイヤーが見つからない場合は処理しない
		return;
	}
	...
	float lerpFactor = 0.1f; // 追従の滑らかさを調整する係数 (0.0で動かない、1.0で即座に追従)
